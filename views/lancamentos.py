        ...
        if meses:
            ...
            def render_editor(df_sub, key_sufixo):
                if not df_sub.empty:
                    ...
                    # Envolvemos em um st.form para que a página não recarregue a cada clique/edição na tabela
                    with st.form(f"form_editor_{key_sufixo}_{mes_escolhido.replace('/', '_')}"):
                                ...
                                for _, row in editor_result.iterrows():
                                    if row['categoria'] not in categorias_entradas and row['categoria'] not in ["Ignorar", "Não sei"]:
                                        ...
                                        if len(termo_limpo) >= 3:
                                            connection.connect().execute( # fallback seguro ou dentro do bloco
                                                text("""
